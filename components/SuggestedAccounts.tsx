...
const SuggestedAccounts = () => {
  ...
  return (
    <div className='xl:border-b-2 border-gray-200 pb-4'>
      ...
      <div>
        {allUsers.slice(0, 6).map((user: IUser) => <Link href={`/profile/${user._id}`} key={user._id}>
          <div className='flex gap-3 hover:bg-primary p-2 cursor-pointer font-semibold rounded items-center'>
            <div className='w-8 h-8'>
              <Image src={user.image} width={34} height={34} alt="Suggested User" className='rounded-full' layout='responsive'/>
            </div>
